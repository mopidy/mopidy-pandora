...
class TestPandoraFrontendListener(object):
    ...
    def test_on_event_forwards_to_specific_handler(self, frontend_listener):
        ...
        frontend_listener.end_of_tracklist_reached.assert_called_with(station_id='id_mock', auto_play=False)

    def test_listener_has_default_impl_for_end_of_tracklist_reached(self, frontend_listener):
        frontend_listener.end_of_tracklist_reached(station_id='id_mock', auto_play=False)
...
class TestPandoraBackendListener(object):
    ...
    def test_on_event_forwards_to_specific_handler(self, backend_listener):
        ...
        backend_listener.next_track_available.assert_called_with(track=models.Ref(name='name_mock'), auto_play=False)

    def test_listener_has_default_impl_for_next_track_available(self, backend_listener):
        backend_listener.next_track_available(track=models.Ref(name='name_mock'), auto_play=False)
...
class TestPandoraPlaybackListener(object):
    ...
    def test_on_event_forwards_to_specific_handler(self, playback_listener):
        ...
        playback_listener.track_changing.assert_called_with(track=models.Ref(name='name_mock'))
